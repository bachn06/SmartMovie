...
final class TopRatedViewController: UIViewController {
    ...
    private var presenter: TopRatedPresenterProtocol?
    ...
    override func viewDidLoad() {
        ...
        presenter?.fetchData(1) {
            DispatchQueue.main.async {
                self.setupCollectionView(self.topRatedCollectionView)
...
extension TopRatedViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        if AppSettings.isGrid {
            let cell = topRatedCollectionView.dequeueCell(indexPath: indexPath) as GridMovieCell
            cell.layer.cornerRadius = 10
            guard let presenter = presenter else { return cell }
            cell.setupCellUI(presenter.cellForItemAt(index: indexPath.item))
            return cell
        } else {
            let cell = topRatedCollectionView.dequeueCell(indexPath: indexPath) as ListMovieCell
            cell.layer.cornerRadius = 10
            guard let presenter = presenter else { return cell }
            cell.setupCellUI(presenter.cellForItemAt(index: indexPath.item))
            return cell
